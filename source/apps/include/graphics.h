    ...
    public:
        ...
        void select (int i)
        {
            ...
            if (i<3) 
            {
                for(int j=0; j<15;j++)
                {
                    tft.line(x + i*(w+d) + w/2 -j  , y-j-2,  x+ i*(w+d)+ w/2 +j, y-j-2, Black);

                    tft.line(x + i*(w+d) + w/2 -j , y2+j+2, x+ i*(w+d)+ w/2 +j, y2+j+2, Black);
                }
            }
            else 
            {
                ...
                for(int j=0; j<15;j++)
                {
                    ...
                    tft.line(x + i*(w+d)+ w/2 -j , y-j-2 +  h  + d,  x+ i*(w+d)+ w/2 +j, y-j-2 +  h  + d, Black);

                    tft.line(x + i*(w+d) + w/2 -j , y2+j+2+  h  + d, x+ i*(w+d)+ w/2 +j, y2+j+2+  h  + d, Black);
                }
            }
        ...
        void set_label(int select, char * label) 
        {
            ...
            if (select<3) 
            {
                tft.locate(select*(w+d) + x+10, y2 - 60);
            }
            else 
            {
                select -= 3;
                tft.locate(select*(w+d) + x+10, y2 -60+h+d);
